    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            cartViewModel.removeCartState.collect { result ->
                when (result) {
                    is TaskResult.Loading -> {
                        "Loading".showToast(this@CartActivity)
                    }

                    is TaskResult.Success -> {
                        "Xóa thành công".showToast(this@CartActivity)
                    }

                    is TaskResult.Error -> {
                        "Thêm thất bại: ${result.exception.message}".showToast(this@CartActivity)
                    }
                }
            }
        }
            ...
            cartViewModel.cart.collect { result ->
                when (result) {
                    ...
                    is TaskResult.Error -> {
                        binding.swipeRefreshCart.isRefreshing = false
                        cartAdapter.updateData(emptyList())
                        Toast.makeText(this@CartActivity, result.exception.message, Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
        binding.btnCheckout.setOnClickListener {
            ...
            if (currentCart is TaskResult.Success && currentCart.data.isNotEmpty()) {
                ...
                startActivity(intent)
                 ...
                 "Giỏ hàng trống hoặc chưa tải xong".showToast(this)
            }
        }
    }
}